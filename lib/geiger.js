...
const notify = (payload) => {
  console.log('CONFIG ', atom.config.get('geiger'))
  let latest
...
export default {
  ...
  activate(state) {
    ...
    timestampLastCheck = JSON.parse(state).timestampLastCheck || 0
    ...
    console.log('input ', Date.now(), timestampLastCheck, bc.throttle);
    console.log('output ', Date.now() - timestampLastCheck, bc.throttle * 60000);
    console.log('computed ', Date.now() - timestampLastCheck > bc.throttle * 60000);
